...
const ScrollVelocity = React.forwardRef<HTMLDivElement, ScrollVelocityProps>(
  ({ children, velocity = 5, movable = true, clamp = false, className, contentClassName, ...props }, ref) => {
    ...
    return (
      ...
      >
        <motion.div
          className={cn(
            "flex flex-row flex-nowrap whitespace-nowrap text-xl font-semibold uppercase *:mr-6 *:block md:text-2xl xl:text-4xl",
            contentClassName,
          )}
          style={{ x }}
        >
